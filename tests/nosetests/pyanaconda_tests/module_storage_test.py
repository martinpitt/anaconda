...
class StorageTasksTestCase(unittest.TestCase):
    ...
    def mount_filesystems_test(self, blivet_run_program, makedirs, core_run_program, mkdirChain):
        ...
        blivet_run_program.assert_any_call(['mount', '-t', 'bind', '-o',
                'bind,defaults', '/dev', '/mnt/sysimage/dev'])
        # remounted the root filesystem
        core_run_program.assert_any_call(['mount', '--rbind', '/mnt/sysimage',
                '/mnt/sysroot'], stdin=None, stdout=None, root='/',
                env_prune=None, log_output=True, binary_output=False)
